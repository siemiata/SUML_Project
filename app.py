...
def credit_score_prediction(income, liabilities, age, employment_type, credit_history):
    ...
    features = [[income, liabilities, age, employment_type_encoded[employment_type], credit_history_encoded[credit_history]]]
...
if menu == "Dodaj klienta":
    ...
    with st.form("add_customer_form"):
        ...
        employment_type = st.selectbox("Rodzaj zatrudnienia", ["UoP", "Zlecenie", "B2B"])
    ...
    if submitted:
        ...
        if customer:
            ...
            credit_score = credit_score_prediction(customer[3], customer[4], customer[5], customer[6], customer[7])
    ...
    with st.form("edit_customer_form"):
        edit_pesel = st.text_input("Podaj PESEL klienta do edycji", value=st.session_state.edit_pesel)
    ...
    if st.session_state.edit_pesel:
        ...
        if customer:
            ...
            with st.form("update_customer_form"):
                ...
                new_age = st.number_input("Nowy wiek", min_value=18, max_value=120, value=customer[5])
                new_employment_type = st.selectbox("Nowy rodzaj zatrudnienia", ["UoP", "Zlecenie", "B2B"],
                                                   index=["UoP", "Zlecenie", "B2B"].index(customer[6]))
                new_credit_history = st.selectbox("Nowa historia kredytowa", ["Dobra", "Średnia", "Zła", "Brak"],
                                                  index=["Dobra", "Średnia", "Zła", "Brak"].index(customer[7]))
                save_submitted = st.form_submit_button("Zapisz zmiany")
                if save_submitted:
                    update_customer(st.session_state.edit_pesel, new_name, new_income, new_liabilities,
                                    new_age, new_employment_type, new_credit_history)
        else:
